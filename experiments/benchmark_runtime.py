"""
benchmark_runtime.py — Benchmarking Waktu Presisi Tinggi & Terstandarisasi
==========================================================================
Mengukur waktu inferensi per-citra secara terstandarisasi dengan metodologi identik:
  - Timer: time.perf_counter()
  - Warmup: 20 iterasi per model
  - Evaluasi single-image pada seluruh 173 citra test set
  - Memisahkan tahapan waktu secara granular:
      * YOLO Wall-Clock (Preprocess, Inference, Postprocess)
      * HOG-KNN GT (Crop, Resize, Grayscale, HOG, KNN Predict)
      * YOLO-HOG-KNN Hybrid (YOLO Det + Crop + HOG + KNN Predict)
  - Menghitung statistik: Mean, Median, Std, P5, P95, FPS (1000/mean)
Output:
  - outputs/runtime_raw.csv
...
from experiments.config import (
    ...
    YOLO_CONF_THRESHOLD, BENCHMARK_DEVICE, BENCHMARK_WARMUP_ROUNDS,
...
def run_runtime_benchmark(best_k=None, X_train=None, y_train=None):
    print("=" * 60)
    print("  BENCHMARKING RUNTIME PRESISI TINGGI (SINGLE-IMAGE INFERENCE)")
    print("=" * 60)
    print(f"  Warmup Rounds : {BENCHMARK_WARMUP_ROUNDS} iterasi")
    print(f"  Device        : GPU CUDA:0 (PyTorch: {torch.cuda.get_device_name(0) if torch.cuda.is_available() else 'CPU'})")
    ...
    print(f"\n[2/4] Loading YOLO model: {MODEL_WEIGHTS_PATH}...")
    yolo_model = YOLO(str(MODEL_WEIGHTS_PATH))

    # 3. Load test images & ground truth annotations into memory to eliminate disk I/O
    print("\n[3/4] Pre-loading 173 test images into RAM (eliminasi latency disk I/O)...")
    ...
    for sample in test_samples:
        ...
        # --- A. YOLO Measurement (Single Image) ---
        if torch.cuda.is_available():
            torch.cuda.synchronize()
        ...
        # Extract internal speed if available
        ...
        # --- B. HOG-KNN Ground-Truth Pipeline ---
        ...
        # --- C. YOLO-HOG-KNN Hybrid Pipeline ---
        # 1. BBox extraction from YOLO results
    ...
    df_raw.to_csv(OUTPUT_DIR / 'runtime_raw.csv', index=False)
    print(f"  [SAVED] {OUTPUT_DIR / 'runtime_raw.csv'}")
    ...
    df_summary.to_csv(OUTPUT_DIR / 'runtime_summary.csv', index=False)
    print(f"  [SAVED] {OUTPUT_DIR / 'runtime_summary.csv'}")

    print("\n" + "=" * 80)
    print("  RINGKASAN BENCHMARK RUNTIME (ms) & FPS")
    print("=" * 80)
