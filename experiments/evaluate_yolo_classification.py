"""
evaluate_yolo_classification.py — Evaluasi Image-Level Classification YOLOv13n
==============================================================================
Menjalankan inferensi citra utuh YOLOv13n pada test set (173 citra).
Memilih deteksi wajah utama (bounding box terbesar) untuk menentukan kelas prediksi.
Menghitung metrik klasifikasi standar agar dapat dibandingkan secara adil dengan KNN.
"""
...
def evaluate_yolo_classification():
    print("=" * 60)
    print("  EVALUASI IMAGE-LEVEL CLASSIFICATION YOLOv13n (TEST SET)")
    print("=" * 60)
    ...
    print(f"\n[1/2] Loading YOLO model: {MODEL_WEIGHTS_PATH}...")
    model = YOLO(str(MODEL_WEIGHTS_PATH))

    print(f"\n[2/2] Inferensi pada {len(img_files)} citra test...")
    predictions = []
    ...
    for img_path in img_files:
        ...
        if face_detected:
            # Pilih deteksi wajah dengan bounding box terbesar
            largest_area = 0
            best_idx = 0
    ...
    summary_metrics = {
        'model_name': 'YOLOv13n Image-Level Classification',
    ...
    # Save outputs
    json_path = OUTPUT_DIR / 'yolo_classification_metrics.json'
    with open(json_path, 'w', encoding='utf-8') as f:
        json.dump(summary_metrics, f, indent=2)
    # Also save to yolo_metrics.json for backward compatibility
    with open(OUTPUT_DIR / 'yolo_metrics.json', 'w', encoding='utf-8') as f:
        json.dump(summary_metrics, f, indent=2)
    print(f"  [SAVED] {json_path}")

    csv_path = OUTPUT_DIR / 'yolo_classification_predictions.csv'
    preds_df.to_csv(csv_path, index=False)
    preds_df.to_csv(OUTPUT_DIR / 'yolo_predictions.csv', index=False)
    print(f"  [SAVED] {csv_path}")

    cm_path = OUTPUT_DIR / 'yolo_classification_confusion_matrix.png'
