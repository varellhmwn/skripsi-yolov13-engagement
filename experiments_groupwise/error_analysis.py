"""
error_analysis.py — Analisis Kesalahan & Visual Error Inspection (Group-Wise)
=============================================================================
Membandingkan prediksi ketiga pendekatan pada test set group-wise dan
menghasilkan visualisasi anotasi per-citra pada folder outputs_groupwise/error_samples/.
Output:
  - outputs_groupwise/error_analysis.csv
  - outputs_groupwise/error_samples/
"""
...
from experiments_groupwise.config import (
    GROUPWISE_DATASET_DIR, OUTPUT_GROUPWISE_DIR
)
from experiments_groupwise.hog_features import parse_yolo_annotation


def run_error_analysis_groupwise():
    print("=" * 65)
    print("  TAHAP 13: ANALISIS KESALAHAN & VISUAL ERROR INSPECTION (GROUP-WISE)")
    print("=" * 65)

    OUTPUT_GROUPWISE_DIR.mkdir(parents=True, exist_ok=True)
    error_samples_dir = OUTPUT_GROUPWISE_DIR / 'error_samples'
    error_samples_dir.mkdir(parents=True, exist_ok=True)

    yolo_preds_path = OUTPUT_GROUPWISE_DIR / 'yolo_classification_predictions.csv'
    knn_gt_preds_path = OUTPUT_GROUPWISE_DIR / 'hog_knn_gt_predictions.csv'
    hybrid_preds_path = OUTPUT_GROUPWISE_DIR / 'yolo_hog_knn_predictions.csv'
    ...
    csv_path = OUTPUT_GROUPWISE_DIR / 'error_analysis.csv'
    ...
    print(f"  [SAVED] {len(misclassified)} visual error samples di {error_samples_dir}")
    return df_error
