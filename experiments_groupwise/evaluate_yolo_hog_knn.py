"""
evaluate_yolo_hog_knn.py — Eksperimen B: YOLO Crop + HOG-KNN (Group-Wise Hybrid)
=================================================================================
Pipeline hybrid:
  1. Full test image -> YOLOv13n retrained (mengambil bounding box wajah terbesar)
...
Kelas hasil deteksi YOLO diabaikan; KNN menentukan kelas emosi.
Output:
  - outputs_groupwise/yolo_hog_knn_metrics.json
  - outputs_groupwise/yolo_hog_knn_predictions.csv
  - outputs_groupwise/yolo_hog_knn_confusion_matrix.png
"""
...
from experiments_groupwise.config import (
    ...
    YOLO_IMGSZ, YOLO_CONF_THRESHOLD, BENCHMARK_DEVICE, CLASS_LIST,
    VALID_IMG_EXTS, KNN_METRIC
)
...
def evaluate_yolo_hog_knn_groupwise(best_k=None, X_train=None, y_train=None):
    print("=" * 65)
    print("  TAHAP 11: EKSPERIMEN B: YOLO CROP + HOG-KNN (GROUP-WISE HYBRID)")
    print("=" * 65)
    ...
    print(f"\n[1/3] Melatih model KNN (K={best_k}) pada {len(X_train)} data latih group-wise...")
    knn = KNeighborsClassifier(n_neighbors=best_k, metric=KNN_METRIC)
    knn.fit(X_train, y_train)

    print(f"\n[2/3] Memuat model YOLO group-wise: {TRAINED_GROUPWISE_WEIGHTS}...")
    ...
    print(f"\n[3/3] Menjalankan pipeline hybrid pada {len(img_files)} citra test group-wise...")
    predictions = []
    ...
    summary_metrics = {
        'model_name': 'YOLO-based Face Crop + HOG-KNN Hybrid (Group-Wise)',
    ...
    json_path = OUTPUT_GROUPWISE_DIR / 'yolo_hog_knn_metrics.json'
    with open(json_path, 'w', encoding='utf-8') as f:
        json.dump(summary_metrics, f, indent=2)
    print(f"  [SAVED] {json_path}")

    csv_path = OUTPUT_GROUPWISE_DIR / 'yolo_hog_knn_predictions.csv'
    preds_df.to_csv(csv_path, index=False)
    print(f"  [SAVED] {csv_path}")

    cm_path = OUTPUT_GROUPWISE_DIR / 'yolo_hog_knn_confusion_matrix.png'
    plot_confusion_matrix(
        class_metrics['confusion_matrix'],
        CLASS_LIST,
        f'YOLO-based Face Crop + HOG-KNN (Group-Wise Test, K={best_k})',
        cm_path,
        accuracy=class_metrics['accuracy']
    )
...
if __name__ == '__main__':
    evaluate_yolo_hog_knn_groupwise()
