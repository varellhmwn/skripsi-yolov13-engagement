"""
evaluate_yolo.py — Evaluasi Native Object Detection YOLOv13n (Group-Wise Test Set)
===================================================================================
Menjalankan evaluasi native object detection model YOLOv13n baru hasil retraining
pada test set group-wise v1 (166 citra).
Output:
  - outputs_groupwise/yolo_detection_metrics.json
  - outputs_groupwise/yolo_detection_confusion_matrix.png
"""
...
from experiments_groupwise.config import (
    TRAINED_GROUPWISE_WEIGHTS, GROUPWISE_DATA_YAML, OUTPUT_GROUPWISE_DIR,
    YOLO_IMGSZ, BENCHMARK_DEVICE, CLASS_LIST
)


def evaluate_yolo_groupwise_detection():
    print("=" * 65)
    print("  TAHAP 8: EVALUASI NATIVE OBJECT DETECTION YOLOv13n (GROUP-WISE)")
    print("=" * 65)
    print(f"  Model Weights : {TRAINED_GROUPWISE_WEIGHTS}")
    ...
    speed = getattr(val_results, 'speed', {})

    detection_metrics = {
        'model_name': 'YOLOv13n (Group-Wise Retrained)',
        'weights_path': str(TRAINED_GROUPWISE_WEIGHTS),
        'dataset_yaml': str(GROUPWISE_DATA_YAML),
        'precision': mp,
        'recall': mr,
        'f1_detection': f1_det,
        'mAP_50': map50,
    ...
    json_path = OUTPUT_GROUPWISE_DIR / 'yolo_detection_metrics.json'
